...
1.  **配置環境變數**: 將 `.env.example` 複製為 `.env`，並填入您的 `SPREADSHEET_ID`。
2.  **配置憑證**: 將您的 Google 服務帳號金鑰 (JSON) 放入 `service_account/google_credentials.json`。
3.  **確認資安**: `.env` 與 `service_account/` 已被加入 `.gitignore`，確保機密資訊不會被上傳。
